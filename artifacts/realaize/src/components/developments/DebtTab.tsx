        ...
        return (
          <div className="animate-fade-in">
            <div className="grid grid-cols-2 gap-6">
              ...
              <GlassPanel style={{ padding: 24 }}>
                ...
                <div className="space-y-4 mt-2">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label style={{ fontSize: 11, fontWeight: 600, color: 'rgba(60,60,67,0.50)', display: 'block', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>LTV (% of purchase price)</label>
                    ...
                    <div>
                      <label style={{ fontSize: 11, fontWeight: 600, color: 'rgba(60,60,67,0.50)', display: 'block', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>LTC (% of construction cost)</label>
                      <div className="flex items-center gap-2">
                        <input type="number" className="input-glass flex-1" min={0} max={100} step={0.5}
                  ...
                  <div>
                    <label style={{ fontSize: 11, fontWeight: 600, color: 'rgba(60,60,67,0.50)', display: 'block', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Interest Rate p.a.</label>
                    <div className="flex items-center gap-2">
                  ...
                  <div>
                    <label style={{ fontSize: 11, fontWeight: 600, color: 'rgba(60,60,67,0.50)', display: 'block', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Loan Type</label>
                    <select className="input-glass w-full" value={debtForm.loanType}
                      ...
                      <option value="Bullet">{de ? 'Bullet Loan (Rückzahlung bei Fertigstellung)' : 'Bullet Loan (repayment at completion)'}</option>
                      <option value="Annuität">{de ? 'Annuitätendarlehen' : 'Annuity Loan'}</option>
                    </select>
                  </div>
                  {debtForm.loanType === 'Annuität' && (
                    <div>
                      <label style={{ fontSize: 11, fontWeight: 600, color: 'rgba(60,60,67,0.50)', display: 'block', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Loan Term (Years)</label>
                  ...
                  {debtForm.loanType === 'Bullet' && completionDate && (
                    <div style={{ padding: '10px 14px', borderRadius: 10, background: 'rgba(0,122,255,0.06)', border: '1px solid rgba(0,122,255,0.12)', fontSize: 12, color: 'rgba(60,60,67,0.65)' }}>
                      {de ? 'Bullet-Rückzahlung zum Abschluss gemäß Gantt:' : 'Bullet repayment at completion per Gantt:'} <strong style={{ color: '#007aff' }}>{new Date(completionDate).toLocaleDateString(dateLocale, { month: 'long', year: 'numeric' })}</strong>
                    </div>
                  )}
                  <button
                  ...
                  >
                    {de ? 'Annahmen speichern' : 'Save Assumptions'}
                  </button>
                </div>
              </GlassPanel>
              ...
              {/* Computed results */}
              <GlassPanel style={{ padding: 24 }}>
                <SectionHeader title="Financing Structure" />
                <div className="space-y-2 mt-2">
                  {[
                    { label: de ? 'Kaufpreis (Basis LTV)' : 'Purchase Price (LTV base)', value: formatEUR(dev.purchasePrice), sub: null },
                    { label: `${de ? 'Kaufpreisdarlehen' : 'Purchase Loan'} (${debtForm.ltvPct}% LTV)`, value: formatEUR(purchaseLoan), color: '#007aff' },
                    { label: de ? 'Baukosten (Basis LTC)' : 'Construction Cost (LTC base)', value: formatEUR(totalBudget), sub: null },
                    { label: `${de ? 'Baukostendarlehen' : 'Construction Loan'} (${debtForm.ltcPct}% LTC)`, value: formatEUR(constructionLoan), color: '#007aff' },
                  ].map(row => (
                  ...
                  <div className="flex justify-between py-3 px-3 rounded-xl" style={{ background: 'rgba(0,122,255,0.07)', border: '1px solid rgba(0,122,255,0.15)', marginTop: 4 }}>
                    <span style={{ fontSize: 14, fontWeight: 700, color: '#1c1c1e' }}>{de ? 'Gesamtdarlehen' : 'Total Debt'}</span>
                  ...
                  <div style={{ borderTop: '1px solid rgba(0,0,0,0.07)', paddingTop: 12, marginTop: 4 }}>
                    <div className="flex justify-between py-2 px-3 rounded-xl" style={{ background: 'rgba(0,0,0,0.03)' }}>
                      <span style={{ fontSize: 13, color: 'rgba(60,60,67,0.65)' }}>{de ? 'Jahreszinsen' : 'Annual Interest'} ({debtForm.interestRatePct}%)</span>
                    ...
                    <div className="flex justify-between py-2 px-3 rounded-xl mt-1" style={{ background: 'rgba(0,0,0,0.03)' }}>
                      <span style={{ fontSize: 13, color: 'rgba(60,60,67,0.65)' }}>{de ? 'Monatliche Zinslast' : 'Monthly Interest'}</span>
                    ...
                    {debtForm.loanType === 'Annuität' && monthlyPayment > 0 && (
                      <div className="flex justify-between py-2 px-3 rounded-xl mt-1" style={{ background: 'rgba(0,0,0,0.03)' }}>
                        <span style={{ fontSize: 13, color: 'rgba(60,60,67,0.65)' }}>{de ? 'Monatliche Annuität' : 'Monthly Annuity'}</span>
                    ...
                    {debtForm.loanType === 'Bullet' && (
                      <div className="flex justify-between py-2 px-3 rounded-xl mt-1" style={{ background: 'rgba(0,0,0,0.03)' }}>
                        <span style={{ fontSize: 13, color: 'rgba(60,60,67,0.65)' }}>{de ? 'Rückzahlung (Bullet)' : 'Repayment (Bullet)'}</span>
                        <span style={{ fontSize: 13, fontWeight: 700, color: '#1c1c1e', fontFamily: 'ui-monospace, monospace' }}>{formatEUR(totalDebt)} {de ? 'bei Fertigstellung' : 'at completion'}</span>
                      </div>
                    )}
                  </div>
                  <div className="flex justify-between py-2 px-3 rounded-xl" style={{ background: 'rgba(0,0,0,0.03)' }}>
                    <span style={{ fontSize: 13, color: 'rgba(60,60,67,0.65)' }}>{de ? 'Eigenkapital (Gesamt)' : 'Equity (Total)'}</span>
